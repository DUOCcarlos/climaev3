...
public class JsonUrlReader2 {
    ...
    public static void main(String[] args) throws DatabindException, MalformedURLException, IOException {
        ...
        ArrayList<TiempoConcepcion> Atp = j.cargarURLx();

        for (TiempoConcepcion tp : Atp) {
            System.out.println(tp);
        }
    }

    public ArrayList<TiempoConcepcion> cargarURLx() throws StreamReadException, DatabindException, MalformedURLException, IOException {
        String url = "https://api.meteored.cl/index.php?api_lang=cl&localidad=18576&affiliate_id=57qkbjgbqx57&v=3.0";
        ArrayList<TiempoConcepcion> tpa = new ArrayList<>();
        ...
        for (int i = 1; i <= 5; i++) {
            ...
            if (horas.isArray()) {
                for (JsonNode hora : horas) {
                    ...
                    TiempoConcepcion tp = new TiempoConcepcion();//aqui hay que crear una clase tiempo concepcion
